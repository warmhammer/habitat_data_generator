...
from hadage.core.lights import change_lights
from hadage.tools.visual import convert_points_to_topdown, display_map, display_sample
from hadage.tools.common import get_state_transform_matrix
...
def run_scenario(sim, sim_settings, light_settings, navigatable_points, logger, display=True, bar_inds=[0, 1], frequent_step=True):
    for nav_index, nav_point in enumerate(tqdm(navigatable_points, desc=f'{logger} proccessing', position=bar_inds[0])):
        ...
        with tqdm(**pbar_kwargs) as pbar:
            _navigation_loop(sim, sim_settings, follower, nav_point, pbar, logger, display, frequent_step)


def _navigation_loop(sim, sim_settings, follower, nav_point, pbar, logger, display, frequent_step):
