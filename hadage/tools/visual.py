...
def get_d3_150_colors_rgb():
    return np.random.default_rng(200).integers(0, 256, (150, 3), dtype=np.uint8)
...
def display_sample(rgb_obs, semantic_obs=None, depth_obs=None):
    ...
    if semantic_obs is not None:
        ...
        semantic_img.putpalette(get_d3_150_colors_rgb().flatten())
    ...
    for i, data in enumerate(arr):
        ...
        # Create a legend for semantic image
        if titles[i] == 'semantic':
            unique_labels = np.unique(semantic_obs)

            legend_elements = [
                Patch(
                    facecolor = get_d3_150_colors_rgb()[label % 150] / 255,
                    edgecolor = 'none',
                    label = f'Class {label}'
